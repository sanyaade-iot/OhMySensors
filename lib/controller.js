...
Controller.prototype.handle = async function(parsedMessage) {
  ...
  if (parsedMessage.messageType === 'internal') {
    ...
    if (!sensorExist) {
      ...
      await P_dbRun('INSERT INTO sensors (node_id, child_id, type, description, last_seen) VALUES (?, ?, ?, ?, ?)', parsedMessage.nodeId, parsedMessage.childSensorId, parsedMessage.subType, parsedMessage.payload, new Date().toISOString());
      ...
      await P_dbRun('UPDATE sensors SET last_seen = ? WHERE node_id = ? AND child_id = ?', new Date().toISOString(), parsedMessage.nodeId, parsedMessage.childSensorId);
    }

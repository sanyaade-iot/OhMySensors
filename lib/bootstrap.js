...
module.exports = function bootstrap(configFile, dataDir, ip, port) {
  ...
  try {
    db.config = require(path.join(process.cwd(), configFile));
  } catch (e) {
  ...
  checkDatabaseFile(path.join(dataDir, 'settings.db'), 'settings');
  checkDatabaseFile(path.join(dataDir, 'data.db'), 'data');

  // When arrived here, files are accessible

  db.settings = dirty(path.join(dataDir, 'settings.db'));
  db.data = new sqlite.Database(path.join(dataDir, 'data.db'));
